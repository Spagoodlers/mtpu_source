...
def get_random_creature_image_by_cmc(cmc):
...
def roll_random_cmc(cmc):
    try:
        cmc = int(cmc)
        if cmc < 0 or cmc > 20:
            messagebox.showerror("Invalid CMC", "Please enter a number between 0 and 20.")
            return

        image_url = get_random_creature_image_by_cmc(cmc)
        ...
        if image_url:
            ...
            if image_path:
                ...
                run_image_printer([image_path])  # Call the image_printer.py script
                delete_temp_image()
...
def create_and_configure_gui():
    ...
    grid_row = 0
    grid_column = 0
    for cmc_value in range(17):  # 0 to 16 (to fit the 5x5 grid)
        ...
        if os.path.exists(icon_path):
            ...
            cmc_button = create_transparent_button_(cmc_buttons_frame, text=str(''), command=lambda cmc=cmc_value: on_cmc_button_click_(cmc))
    ...
    output_label_ = tk.Label(app, text="", bg="black", fg="white")  # Set text color to white
    output_label_.pack()
    ...
    checkbox = ttk.Checkbutton(app, text="Enable Printing", variable=checkbox_var_, command=on_checkbox_click_)
    checkbox.pack()
